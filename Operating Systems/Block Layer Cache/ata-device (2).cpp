...
size_t ATADevice::block_size() const
{
	...
	/**
	 * Returns true if block was read successfully and also takes care of caching if block is not present in the cache.
	 * @param buffer pointer to address of buffer.
	 * @param offset used as a key for the map in the cache.
	 * @param count number of blocks to read , <=1 in this case.
	 * @return Returns true if block was read successfully
	 */
bool ATADevice::read_blocks(void* buffer, size_t offset, size_t count)
{

	bool successful = false; // used to indicate if read was successful or not 

	if(_cache.contains(offset,buffer)){ // contains checks if block is in memory and returns that block to the address of the buffer
		ata_log.messagef(LogLevel::DEBUG,"cache: [CACHE HIT] offset = %d",offset);
		successful = true;

	}else{
		successful = transfer(0,offset,buffer,count);
		ata_log.messagef(LogLevel::DEBUG,"cache: [CACHE MISS] offset = %d",offset);
		// replace addLRU with addMRU to use the MRU policy (advanced section )
		_cache.addMRU(offset,buffer); // if the cache doesnt have the block , we add it based on an LRU policy which can be seen in the add() implementation
