    ...
    def _packet_in_handler(self, ev):
        ...
        if tcph and iph:
            ...
            if in_port == 1:
                out_port = 2
                any_flag = (
                    tcph[0].has_flags(tcp.TCP_SYN)
                    or tcph[0].has_flags(tcp.TCP_RST)
                    or tcph[0].has_flags(tcp.TCP_FIN)
                    or tcph[0].has_flags(tcp.TCP_PSH)
                    or tcph[0].has_flags(tcp.TCP_ACK)
                    or tcph[0].has_flags(tcp.TCP_URG)
                    or tcph[0].has_flags(tcp.TCP_ECE)
                    or tcph[0].has_flags(tcp.TCP_CWR)
                )
                # dropping if invalid flag combination
                if ( 
                    tcph[0].has_flags(tcp.TCP_FIN, tcp.TCP_SYN)
                    or tcph[0].has_flags(tcp.TCP_RST, tcp.TCP_SYN)
                    or not any_flag
                ):
